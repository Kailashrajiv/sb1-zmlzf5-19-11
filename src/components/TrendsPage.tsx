...
type PriceRange = 'all' | 'mcx' | 'lme';

const generateMockData = (range: TimeRange) => {
  const data = [];
  const now = new Date();
  const points = 100; // Number of data points to show
  ...
  for (let i = points; i >= 0; i--) {
    ...
    switch (range) {
      ...
      case '1h':
        ...
        break;
      case '1d':
        date = subHours(now, i * 2); // 48 points for a day
        break;
      case '1w':
        date = subDays(now, i);
        break;
      case '1m':
        date = subDays(now, i * 2); // 60 points for a month
        break;
      default:
        date = new Date();
    }
...
export default function TrendsPage() {
  ...
  const [yAxisInterval, setYAxisInterval] = useState(1); // Interval in rupees
